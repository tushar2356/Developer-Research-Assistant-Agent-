...
class Workflow:
    ...
    def _extract_tools_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        messages = [
            ...
            HumanMessage(
                content=self.prompts.tool_extraction_user(
                    state.query,
                    all_content
                )
            )
    ...
    def _analyze_company_content(
        ...
        messages = [
            SystemMessage(
                content=self.prompts.TOOL_ANALYSIS_SYSTEM
            ),
            HumanMessage(
    ...
    def _research_step(
        ...
        if not extracted_tools:
            ...
            try:
                ...
                print(f"Search result parsing error: {e}")
                tool_names = []
        ...
        for tool_name in tool_names:
            ...
            try:
                tool_search_results = (
                    self.firecrawl.search_companies(
                ...
                if (
                    tool_search_results
                    and hasattr(tool_search_results, "web")
                    and len(tool_search_results.web) > 0
                ):
                    ...
                    if (
                        scraped
                        and hasattr(scraped, "markdown")
                    ):

                        content = scraped.markdown

                        analysis = (
                            self._analyze_company_content(
                                company.name,
                                content
                            )
                        )
                        ...
                        company.api_available = (
                            analysis.api_available
                        )

                        company.language_support = (
    ...
    def _analyze_step(
        ...
        messages = [
            SystemMessage(
                content=self.prompts.RECOMMENDATIONS_SYSTEM
            ),
            HumanMessage(
